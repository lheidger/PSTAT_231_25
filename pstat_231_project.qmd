...
osm_census_clean <- osm_census %>% select(-bicycle, -cycleway) %>% drop_na()
...
```{r}
osm_census_clean <- readRDS(here("osm_census_clean.rds"))
osm_census_split <- initial_split(osm_census_clean, prop = 0.80, strata = comfort_level)
